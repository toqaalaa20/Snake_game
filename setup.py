...
def gameLoop():
    ...
    while not gameExit:
        ...
        """
        if lead_x>= randApplex and lead_x<= randApplex+ Apple_thick:
            if lead_y>= randAppley and lead_y<= randAppley+ Apple_thick:
                randApplex= round(random.randrange(0, display_width- block_size))#/10.0)*10.0
                randAppley= round(random.randrange(display_height- block_size))#/10.0)*10.0
                snakelen+=1
        """
        
        if lead_x > randApplex and lead_x <randApplex+ Apple_thick or lead_x+block_size >randApplex and lead_x +block_size < randApplex+Apple_thick:
            if lead_y> randAppley and lead_y< randAppley+ Apple_thick:
                randApplex, randAppley= randApple()
                snakelen+=1
                
            elif lead_y +block_size >randAppley and lead_y+block_size <randAppley +Apple_thick:
                randApplex, randAppley= randApple()
                snakelen+=1  
